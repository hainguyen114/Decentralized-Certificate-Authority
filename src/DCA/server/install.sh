...
FILE="$HOME/.multichain/chaindemo/params.dat"
...
# Please run "multichain-util clone chaindemo <new-network-name>" to generate new network. 


# Basic chain parameters

chain-protocol = multichain             # Chain protocol: multichain (permissions, native assets) or bitcoin
chain-description = MultiChain chaindemo # Chain description, embedded in genesis block coinbase, max 90 chars.
...
anyone-can-create = true               # Anyone can create new streams.
anyone-can-issue = true                # Anyone can issue new native assets.
anyone-can-mine = true                 # Anyone can mine blocks (confirm transactions).
anyone-can-activate = true             # Anyone can grant or revoke connect, send and receive permissions.
...
allow-arbitrary-outputs = true         # Allow arbitrary (without clear destination) scripts.
...
default-network-port = 4771             # Default TCP/IP port for peer-to-peer connection with other nodes.
default-rpc-port = 4770                 # Default TCP/IP port for incoming JSON-RPC API requests.
chain-name = chaindemo                  # Chain name, used as first argument for multichaind and multichain-cli.
protocol-version = 20009                # Protocol version at the moment of blockchain genesis.
network-message-start = f0daf5ec        # Magic value sent as the first 4 bytes of every peer-to-peer message.
address-pubkeyhash-version = 0019e230   # Version bytes used for pay-to-pubkeyhash addresses.
address-scripthash-version = 059d6b57   # Version bytes used for pay-to-scripthash addresses.
private-key-version = 800d5a2f          # Version bytes used for exporting private keys.
address-checksum-value = 63013188       # Bytes used for XOR in address checksum calculation.
...
./storj-sim network setup --host $1
